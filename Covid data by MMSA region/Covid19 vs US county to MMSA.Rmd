...
```{r}
covid19deaths[is.na(covid19deaths),]
covid19cases[is.na(covid19cases),]
```
...
names(county_MMSA) <- c("MMSA_State", "County", "State", "ST_FIPS", "County_FIPS", "Central_Outlying")
county_MMSA$MMSA <- gsub(",.*","", county_MMSA$MMSA_State)
str(county_MMSA)
```
...
The counts remain the same after the merge, but there are some N/As in the dataset.
...
```{r}
covid_location[is.na(covid_location$County_FIPS),]
```
...
I will export the data so that I can create GIS Maps in Tableau. 

```{r}
write.csv(covid_data, 'covid_MMSA.csv')
```
