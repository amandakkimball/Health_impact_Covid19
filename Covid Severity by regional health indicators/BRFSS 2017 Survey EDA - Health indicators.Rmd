...
```{r}
library(tidyr)
MMSApivotHealth <- pivot_longer(data = MMSAHealth, 
                                cols = c(ends_with("weight"), "obese", "extremeobesity",  
                                "Gen_Hlth_bad", "Uninsured", ends_with("high"), ends_with("yes"), 
                                starts_with("age")),
                                names_to = "Indicator name",
                                values_to = "Percent of population")
                                #na.rm = TRUE)

MMSApivotHealth <- MMSApivotHealth[c(1,21:22)]
MMSApivotHealth[3] <- MMSApivotHealth[3]*100
MMSApivotHealth
```
...
write.csv(MMSApivotHealth, 'healthindicatorpivotdata.csv', row.names = FALSE)
```
